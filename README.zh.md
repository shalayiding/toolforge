...
└── community/               ← 预置工具，随 git 分发，所有用户共享
    ├── *_tools.py           ← 纯 Python 工具模块（见 TOOLS.md）
...
> 全新安装后注册表是空的，这是正常的。运行 `uv run python registry/seed_registry.py --community` 加载社区工具（见 [community/TOOLS.md](community/TOOLS.md)），或用 `/toolforge` 添加任意 GitHub 仓库。
...
仓库内置多个开箱即用的社区工具，完整列表见 [community/TOOLS.md](community/TOOLS.md)，无需消耗 token：
